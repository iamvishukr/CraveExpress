...
const ItemList = ({ items, isCartPage = false }) => {
  ...
  return (
    <div>
      {items.map((item) => (
        <div key={item?.card?.info?.id} className="p-2 mt-8 border-gray-800  bg-orange-400 hover:shadow-yellow-500 shadow-xl rounded-2xl border-b-2 text-left">
          ...
          <div className="p-2 flex justify-between items-center">
            <span className="font-bold text-xl text-left ml-0">
              {item?.card?.info?.name}
            ...
            <span className="relative border-b-8 border-r-8 rounded-b-xl rounded-r-xl border-b-yellow-500 border-r-yellow-500">
              <img 
              ...
              {isCartPage ? (
                <button
                  className="absolute bottom-0 right-0 p-2 bg-red-500 text-black text-center shadow-xl rounded-lg"
